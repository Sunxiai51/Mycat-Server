...
public class MultiNodeCoordinator implements ResponseHandler {
            ...
            switch (mysqlCon.getXaStatus()) {
                case TxState.TX_STARTED_STATE:
                    ...
                    if (mysqlCon.batchCmdFinished()) {
                        ...
                        //51 bug fix start
                        if (null == coordinatorLogEntry) {
                            LOGGER.info("bug fix log: coordinatorLogEntry is null, put it.");
                            putLogEntry();
                            coordinatorLogEntry = inMemoryRepository.get(xaTxId);
                        }
                        //51 bug fix end
     ...
     * 设置LogEntry
     * 
     * <p>在 {@link MultiNodeCoordinator#okResponse(byte[], BackendConnection)}的语句
     * 
     * <pre>CoordinatorLogEntry coordinatorLogEntry = inMemoryRepository.get(xaTxId);</pre>
     * 中得到的coordinatorLogEntry可能为空，检测到为空时执行该方法。
     * 
     * @author 51
     */
    private void putLogEntry() {
        final int initCount = session.getTargetCount();
        //recovery nodes log
        ParticipantLogEntry[] participantLogEntry = new ParticipantLogEntry[initCount];
        // 下标
        ...
        if (session.getXaTXID() != null) {
            CoordinatorLogEntry coordinatorLogEntry = new CoordinatorLogEntry(session.getXaTXID(), false, participantLogEntry);
            LOGGER.debug("set coordinatorLogEntry, with participantLogEntry=" + participantLogEntry);
            inMemoryRepository.put(session.getXaTXID(), coordinatorLogEntry);
        }
    }
